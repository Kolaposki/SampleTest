...
<template>
      ...
      <!-- </template> -->
          ...
          ></v-select>
              ...
              >
                ...
                <template v-slot:input>
                  <v-text-field
                      ...
                      label="Edit Qty"
                      single-line
					  :max="props.item.quantityreceived"
                      autofocus
                  ></v-text-field>
                </template>
              </v-edit-dialog>
			...
			<template v-slot:item.recievedate="props">
              <v-edit-dialog                  
                  @cancel="cancel"
                  @open="open"
                  @close="close"
			...
			<template v-slot:item.quantityreceived="props">
				<div>{{ formNumber(props.item.quantityreceived) }}</div>
			</template>
			<template v-slot:item.quantityleft="props">
				<div>{{ formNumber(props.item.quantityleft) }}</div>
			</template>
...
export default {
	...
	data () {
		return {
			...
			headers: [
				...
				// 	sortable: false },
				{ text: 'Qty. Recieved', value: 'quantityreceived', align: 'end',
					sortable: false },
				{ text: 'Qty. Returned', value: 'quantity', align: 'start',
					sortable: false },
				{ text: 'Left Over', value: 'quantityleft', align: 'end',
					sortable: false },
				// { text: 'Cost Per Pack', value: 'packcost', align: 'start',
				// 	sortable: false },
				{ text: 'Return Date', value: 'recievedate', align: 'start',
					sortable: false },
	...
	methods: {
		...
		calculateTotal (product, type) {
			...
			product.quantityleft = product.quantityreceived - product.quantity

			product.totalprice = ((product.quantity * product.unitprice) - this.calcDiscount(product))
			
			// product.totalprice = (product.quantity * product.unitprice) - ((this.calcDiscount(product) / product.quantityreceived) * product.quantity)
