  ...
  <div class="main-content-wrap">
    ...
    <div class="inner-main-wrapper">
      <div class="d-flx-alc-jsb table-title">
        <h3 class="section-title">Save Receive Note</h3>
      </div>
        ...
        <div class="mt-2">
          ...
          <div class="flex-width-1-2 less-gp">
            <label  class="form-input">
              ...
              <div class="form-input__input">
                <select v-model="formData.branchID" class="filter-btn box-deco-smallr disable">
            ...
            <label  class="form-input">
              ...
              <div class="form-input__input">
                <select v-model="formData.vendorID" class="filter-btn box-deco-smallr disable">
          ...
          <div class="flex-width-1-2 less-gp">
            <label  class="form-input">
			  ...
			  <span v-if="errorData.orderdate" class="form-input__error-msg">Please select a Recieve Date</span>
            </label>

            <label  class="form-input">
              <span class="form-input__label">Due date</span>
              <div class="form-input__input">
                <input type="date" v-model="formData.duedate" name="" class="disable" />
              </div>
            </label>
          </div>
          ...
          <RecieveNotes :poitems="poitems" ref="recievenote" :type="'recieveNotes'" />
          ...
          <div class="d-flx-alc-jfe mt-3" id="footer">
            ...
            <button type="submit" id="submitBtn" class="btn btn--primary ml-1" @click="saveReceiveNote">
              + Save Receive Note
            </button>
          </div>
...
<script>
import RecieveNotes from '../../../../components/RecieveNotes'
export default {
	layout: 'main',
	components: {
		RecieveNotes
	...
	computed :{
		recievedate : {
			set (val) {
				this.$refs.recievenote.setDate(val)
	...
	methods: {
		validate() {
			...
			if (products.length > 0) {
				this.$toast.error("Recieve Date for products with the vendor codes "+ products.map(res => res.productname).reduce((acc, cur) => {
					acc += ' '+curr
					return acc
				})  +" are empty")
				return true
		...
		saveReceiveNote() {
			...
			this.$store.dispatch('purchaseorders/saveReceiveNote', this.formData)
				.then(res =>{
					$('#submitBtn').attr('disabled', false).html('+ Save Receive Note')
					if(res.data.status) {
		...
		setReceiveNote() {
			...
			let recievenotes = JSON.parse(localStorage.getItem('receiveNote'))
			if(purchaseorder) {
				...
				// 
				console.log(recievenotes)
				let purchase = purchaseorder.purchaseorderitems;

				this.purchaseorder = purchaseorder.purchaseordercode
        
				let purchaseorderdetails = recievenotes.receivenoteitems
				if (purchaseorderdetails.length > 0) {
					for (let i = 0; i < purchaseorderdetails.length; i++) {
					  ...
					  this.poitems.push({
							...
							outstanding: quantityordered - purchaseorderdetails[i].quantity,
					...
					$("#footer").hide();
					$('.section-title').html('Goods Recieve Note For Purchase Order : '+purchaseorder.purchaseordercode)
				} 
				// else {
				// 	this.addChild()
				// 	this.removeChild(1)
				...
				if (purchaseorderdetails.length > 0) {
					for (let i = 0; i < purchaseorderdetails.length; i++) {
					  this.poitems.push({
							productID: purchaseorderdetails[i].productID,
							quantity: purchaseorderdetails[i].quantity,
							quantityordered: purchaseorderdetails[i].quantity,
							outstanding: 0,
							vendorproductcode: purchaseorderdetails[i].vendorproductcode,
							unitprice: purchaseorderdetails[i].unitprice,
							totalprice: purchaseorderdetails[i].totalprice,
